...
class TIX_Event_Page {
    ...
    private static function render_header($id) {
        ...
        <div class="tix-ep-header">
            ...
            <?php self::render_share($id); ?>
        </div>
        <?php
    }

    /* ── Share-Buttons ── */
    private static function render_share($id) {
        $s = tix_get_settings();
        if (empty($s['ep_show_share'])) return;

        $url   = urlencode(get_permalink($id));
        $title = urlencode(get_the_title($id));
        $raw_url = esc_attr(get_permalink($id));
        ?>
        <div class="tix-ep-share">
            <span class="tix-ep-share-label">Teilen</span>
            <div class="tix-ep-share-buttons">
                <a href="https://wa.me/?text=<?php echo $title; ?>%20<?php echo $url; ?>" target="_blank" rel="noopener"
                   class="tix-ep-share-btn tix-ep-share-wa" title="WhatsApp">
                ...
                <a href="https://www.facebook.com/sharer/sharer.php?u=<?php echo $url; ?>" target="_blank" rel="noopener"
                   class="tix-ep-share-btn tix-ep-share-fb" title="Facebook">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                </a>
                <a href="https://twitter.com/intent/tweet?text=<?php echo $title; ?>&url=<?php echo $url; ?>" target="_blank" rel="noopener"
                   class="tix-ep-share-btn tix-ep-share-x" title="X">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                </a>
                <a href="mailto:?subject=<?php echo $title; ?>&body=<?php echo $url; ?>"
                   class="tix-ep-share-btn tix-ep-share-email" title="E-Mail">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>
                </a>
                <button type="button" class="tix-ep-share-btn tix-ep-share-copy" data-url="<?php echo $raw_url; ?>" title="Link kopieren">
