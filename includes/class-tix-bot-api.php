...
/**
 * TIX_Bot_API – REST-Endpunkte fuer den Tixomat-Chatbot.
 *
 * Konvertiert aus mu-plugins/tix-bot-api.php in eine Plugin-Klasse.
 * Auth liest aus tix_get_settings('bot_api_secret').
 */
class TIX_Bot_API {
    ...
    public static function register_routes() {
        ...
        register_rest_route($ns, '/tickets/lookup', [
            'methods'             => 'POST',
            'callback'            => [__CLASS__, 'tickets_lookup'],
            'permission_callback' => [__CLASS__, 'auth'],
        ]);
        ...
        register_rest_route($ns, '/cart/checkout-url', [
            'methods'             => 'POST',
            'callback'            => [__CLASS__, 'checkout_url'],
            'permission_callback' => [__CLASS__, 'auth'],
        ]);
        ...
        register_rest_route($ns, '/customer/exists', [
            'methods'             => 'GET',
            'callback'            => [__CLASS__, 'customer_exists'],
            'permission_callback' => [__CLASS__, 'auth'],
        ]);
    }
    ...
    public static function auth(WP_REST_Request $req) {
        $secret = function_exists('tix_get_settings') ? tix_get_settings('bot_api_secret') : '';
        $header = $req->get_header('X-Bot-Secret');
    ...
    public static function tickets_lookup(WP_REST_Request $req) {
        $body  = $req->get_json_params();
        $email = sanitize_email($body['email'] ?? '');
        $type  = sanitize_text_field($body['verification_type'] ?? '');
    ...
    public static function checkout_url(WP_REST_Request $req) {
        $body  = $req->get_json_params();
        $items = $body['items'] ?? [];
        ...
        if (!function_exists('wc_get_checkout_url')) {
            return new WP_Error('tixbot_no_wc', 'WooCommerce ist nicht aktiv.', ['status' => 500]);
        }
    ...
    public static function customer_exists(WP_REST_Request $req) {
     ...
     */
    public static function current_price($cat) {
