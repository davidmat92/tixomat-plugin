...
define('TIXOMAT_VERSION', '1.38.308');
...
// ── Migration: ai_guard_api_key → anthropic_api_key ──
add_action('admin_init', function() {
    $s = get_option('tix_settings', []);
    if (!empty($s['ai_guard_api_key']) && empty($s['anthropic_api_key'])) {
        $s['anthropic_api_key'] = $s['ai_guard_api_key'];
...
// ── Admin Dashboard ──
require_once TIXOMAT_PATH . 'includes/class-tix-dashboard.php';
TIX_Dashboard::init();

// ── Rechtstexte (Impressum, Datenschutz, AGB, Zahlungsweisen, Widerruf) ──
require_once TIXOMAT_PATH . 'includes/class-tix-legal.php';
TIX_Legal::init();

// ── Vorbestellungen (Quotes / Magic-Link für Kunden mit Sonderpreisen) ──
require_once TIXOMAT_PATH . 'includes/class-tix-quotes.php';
TIX_Quotes::init();
...
// ── Kunden-Merge (2 WP-Users zusammenfuehren) ──
// TEMPORAER DEAKTIVIERT (v1.38.255) — kritischer Fehler beim User gemeldet, wird untersucht.
// require_once TIXOMAT_PATH . 'includes/class-tix-user-merge.php';

// ── Rundmail an Kunden (Broadcast) ──
require_once TIXOMAT_PATH . 'includes/class-tix-broadcast.php';

// ── IP-Sperrliste (Bot-/Abuse-Abwehr) ──
require_once TIXOMAT_PATH . 'includes/class-tix-ip-block.php';

// ── Geschenkgutscheine (Guthaben-Tickets, online + Scan einloesbar) ──
require_once TIXOMAT_PATH . 'includes/class-tix-giftcards.php';
...
require_once TIXOMAT_PATH . 'includes/class-tix-app-events.php';
TIX_App_Events::init(); // Event-Editor + Bestell-Details für den Veranstalter-Bereich der App
TIX_Music::init(); // Musikwünsche: Deezer-Autocomplete, Wünsche nur während Events, DJ-Link + App
require_once TIXOMAT_PATH . 'includes/class-tix-loyalty.php';
TIX_Loyalty::init(); // Treue-/Prämienprogramm: Stempel per Einlass-QR, Prämien einlösen
...
// ── Rechnungs-System (pluggable Provider, immer geladen) ──
require_once TIXOMAT_PATH . 'includes/class-tix-invoicing.php';
require_once TIXOMAT_PATH . 'includes/class-tix-invoice-provider-lexoffice.php';
TIX_Invoicing::register_provider(
    TIX_Invoice_Provider_Lexoffice::ID,
    TIX_Invoice_Provider_Lexoffice::NAME,
...
if (!empty(tix_get_settings('bot_enabled'))) {
    require_once TIXOMAT_PATH . 'includes/class-tix-bot-api.php';
    TIX_Bot_API::init();
}
...
// Coupon-Popup für Auto-Apply-Coupons
require_once TIXOMAT_PATH . 'includes/class-tix-coupon-popup.php';
...
add_action('init', ['TIX_Coupon_Popup',             'init']);
...
// ── KI-Assistent (AI Writer) ──
require_once TIXOMAT_PATH . 'includes/class-tix-ai-writer.php';
TIX_AI_Writer::init();

// ── Register Event ("Event in 2 Minuten") ──
require_once TIXOMAT_PATH . 'includes/class-tix-register-event.php';
TIX_Register_Event::init();
...
// ── Sponsor-System (immer geladen — keine Setting-Gate) ──
...
if (tix_get_settings('campaign_tracking_enabled')) {
    ...
    // Site-weites Pageview-Tracking + Analytics-Tab
    require_once TIXOMAT_PATH . 'includes/class-tix-site-analytics.php';
    TIX_Site_Analytics::init();
}
...
// ── E-Mail-Tracking (Pixel + Feedback-Endpoints) ──
require_once TIXOMAT_PATH . 'includes/class-tix-email-tracking.php';
TIX_Email_Tracking::init();
...
TIX_Support_Templates::init();
require_once TIXOMAT_PATH . 'includes/class-tix-support-ai.php';
TIX_Support_AI::init();
...
add_action('admin_init', function() {
    ...
    // v1.38.184: Sponsor.coupon_code Spalte (dbDelta-ALTER)
    if (version_compare($stored, '1.38.184', '<')) {
        if (class_exists('TIX_Sponsor_DB')) {
            TIX_Sponsor_DB::create_tables();
        }
    }

    // v1.38.186: Sponsor.password_hash Spalte (dbDelta-ALTER)
    if (version_compare($stored, '1.38.186', '<')) {
        if (class_exists('TIX_Sponsor_DB')) {
            TIX_Sponsor_DB::create_tables();
        }
    }
